/** Small fail-closed UTC cron expression parser for minute-granularity schedules. */

const FIELD_RANGES: readonly CronFieldRange[] = [
  { min: 0, max: 59 },
  { min: 0, max: 23 },
  { min: 1, max: 31 },
  { min: 1, max: 12 },
...
export class CronExpression {
  ...
  constructor(expression: string) {
    ...
    if (fields.length !== 5) {
      throw new CronExpressionError("cron schedule must contain exactly 5 fields");
    }
    this.#fields = fields.map((field, index) => parseField(field, requireRange(index)));
  }
  ...
  matches(date: Date): boolean {
    ...
    return values.every((value, index) => this.#fields[index]?.has(value) ?? false);
  }
}

function requireRange(index: number): CronFieldRange {
  const range = FIELD_RANGES[index];
  if (range === undefined) throw new CronExpressionError("missing cron field range");
  return range;
}

function parseField(raw: string, range: CronFieldRange): ReadonlySet<number> {
  const values = new Set<number>();
  for (const part of raw.split(",")) addPart(values, part, range);
  if (values.size === 0) throw new CronExpressionError(`empty cron field: ${raw}`);
  return values;
}

function addPart(values: Set<number>, raw: string, range: CronFieldRange): void {
...
function parseBase(raw: string, range: CronFieldRange): readonly [number, number] {
  ...
  if (dash >= 0) {
    ...
    if (start > end) throw new CronExpressionError(`invalid descending range: ${raw}`);
    return [start, end];
  }
...
function parsePositiveInt(raw: string, label: string): number {
  ...
  if (value <= 0) throw new CronExpressionError(`cron ${label} must be positive: ${raw}`);
  return value;
}

function parsePositiveOrZeroInt(raw: string, label: string): number {
  if (!/^\d+$/.test(raw)) throw new CronExpressionError(`invalid cron ${label}: ${raw}`);
  return Number(raw);
}
