/**
 * ExecutionPolicy factory and validation.
 *
 * Creates bounded generic `ExecutionPolicy` objects and worker-layer
 * `WorkerPolicy` objects with sensible defaults. The policy is enforced by
 * the runtime, not by agent judgment.
 *
 * @module pi-tools/execution-policy
 */

import path from "node:path";
import type { CredentialAccessLevel, ExecutionPolicy, WorkerPolicy } from "@pi-crew/core";
...
const DEFAULT_ROOT_PATH = "/tmp/pi-worker";
...
/**
 * Partial policy input used to construct a full {@link ExecutionPolicy}.
 *
 * All fields are optional except `policyId`; sensible defaults are applied by
 * {@link createExecutionPolicy}.
 */
...
/** Worker-specific factory input layered on {@link ExecutionPolicyInput}. */
export interface WorkerPolicyInput extends Omit<ExecutionPolicyInput, "policyId" | "rootPath"> {
  readonly assignmentId: string;
  readonly runId: string;
  readonly taskId: string;
  readonly role: string;

  // Filesystem compatibility for worker assignments.
...
/**
 * Create a fully-populated {@link ExecutionPolicy} from partial input,
...
/**
 * Create a worker-specific policy from Den assignment data.
 *
 * For workers, `policyId` is the Den assignment ID and `workdir` mirrors the
 * generic `rootPath` for backward compatibility with existing runtime code.
 */
export function createWorkerPolicy(input: WorkerPolicyInput): WorkerPolicy {
  const rootPath = input.rootPath ?? input.workdir ?? DEFAULT_ROOT_PATH;
  const executionPolicy = createExecutionPolicy({
    policyId: input.assignmentId,
    rootPath,
    allowedPaths: input.allowedPaths,
    denyPaths: input.denyPaths,
    allowedTools: input.allowedTools,
    ...
    maxTurnDurationMs: input.maxTurnDurationMs,
...
export function isPathAllowed(
  ...
  const resolved = resolvePolicyPath(policy.rootPath, targetPath);

  for (const deny of policy.denyPaths) {
    if (isWithinOrEqual(resolved, resolvePolicyPath(policy.rootPath, deny))) {
      return false;
    }
  }
  ...
  return allowedRoots.some((allowed: string) =>
    isWithinOrEqual(resolved, resolvePolicyPath(policy.rootPath, allowed)),
  );
}

export function resolvePolicyPath(rootPath: string, inputPath: string): string {
 ...
 */
export function isHostAllowed(policy: ExecutionPolicy, host: string): boolean {
