/**
 * Full-agent session execution policy derivation.
 *
 * Derives an {@link ExecutionPolicy} from profile/config input for ordinary
 * full-agent sessions. Worker policy derivation is unchanged; see
 * {@link createWorkerPolicy} in execution-policy.ts.
 *
 * DESIGN: Full-agent sessions get a more permissive path/workspace policy
 ...
 *
 * @module pi-tools/full-agent-policy
 */
 ...
 * appear in a full-agent session's tool surface.
 */
...
/**
 * Partial policy input for constructing a full-agent session policy.
 *
 * All fields are optional; sensible defaults are applied by
 * {@link createFullAgentPolicy}.
 */
...
/**
 * Create an {@link ExecutionPolicy} for an ordinary full-agent session.
 *
 ...
 * in a fullAgent tool surface.
 */
export function createFullAgentPolicy(
  input: FullAgentPolicyInput,
): ExecutionPolicy {
