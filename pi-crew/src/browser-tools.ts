...
const DEFAULT_CHROME = process.env.PI_CREW_CHROMIUM_PATH ?? "chromium";
const WS_OPEN = 1;

export function createBrowserTools(input: { readonly sessionId: string; readonly profileId: string }): AgentTool[] {
  return [
    browserNavigateTool(input),
    browserSnapshotTool(input),
    browserClickTool(input),
    browserTypeTool(input),
    browserScrollTool(input),
    browserBackTool(input),
    browserPressTool(input),
    browserConsoleTool(input),
    browserVisionTool(input),
  ];
}

function browserNavigateTool(input: ToolInput): AgentTool {
  return tool("browser_navigate", "Navigate the session-scoped Chromium page to a URL.", { url: stringSchema("HTTP(S) URL to open.") }, ["url"], async (params) => {
    const browser = await session(input);
    await browser.call("Page.navigate", { url: stringParam(params, "url") });
    await browser.call("Page.loadEventFired", {}, 8_000).catch(() => undefined);
    return ok("navigated", { url: stringParam(params, "url") });
  });
}

function browserSnapshotTool(input: ToolInput): AgentTool {
...
function browserClickTool(input: ToolInput): AgentTool {
...
function browserTypeTool(input: ToolInput): AgentTool {
...
function browserScrollTool(input: ToolInput): AgentTool {
...
function browserBackTool(input: ToolInput): AgentTool {
...
function browserPressTool(input: ToolInput): AgentTool {
...
function browserConsoleTool(input: ToolInput): AgentTool {
...
function browserVisionTool(input: ToolInput): AgentTool {
...
async function session(input: ToolInput): Promise<CdpClient> {
  ...
  next.process = spawn(DEFAULT_CHROME, ["--headless=new", "--remote-debugging-port=0", `--user-data-dir=${next.userDataDir}`, "--disable-gpu", "--no-first-run", "about:blank"], { stdio: ["ignore", "pipe", "pipe"] });
  ...
  next.cdp = await CdpClient.connect(next.pageWebSocketUrl);
...
class CdpClient {
  ...
  call(method: string, params: Record<string, unknown>, timeoutMs = 15_000): Promise<unknown> {
