...
import {
  DenPoolCleanupError,
  reconcileStaleGroupMembers,
  type DenPoolCleanupGroup,
} from "./den-pool-cleanup.js";
...
class McpDenPoolMemberReconciler implements DenPoolMemberReconciler {
  ...
  async #quarantineStaleGroupMembers(): Promise<string[]> {
    try {
      return await reconcileStaleGroupMembers({
      ...
      if (error instanceof DenPoolCleanupError) {
        throw new DenPoolSourceConfigurationError(error.message);
      }
      throw error;
    }
  }
