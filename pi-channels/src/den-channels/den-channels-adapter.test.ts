...
describe("DenChannelsAdapter", () => {
  ...
  // ── ACCEPTANCE: disconnect/reconnect with typed events ────────
  ...
  it("emits reconnecting event during reconnection", async () => {
    ...
    // Simulated connection doesn't auto-reconnect, but we can verify
    // the listener registration and event type signature
    await adapter.connect();

    // Emit directly to verify types
    simConn.simulateDisconnect("transient error");
    // The simulated connection doesn't have built-in reconnect logic;
    // the real DenWebSocketConnection does.  The test verifies that
    // the adapter wires up the reconnecting listener correctly.
  });
  ...
  it("emits connectionFailed event when reconnect exhausted", async () => {
    ...
    // Direct emit to verify the adapter wires this up
    // In real scenario, DenWebSocketConnection emits this after
  ...
  it("adapter disconnect unsubscribes all connection listeners", async () => {
    ...
    // The adapter's listener was unsubscribed, but the direct
    // simConn listener still fires — that's correct behavior.
  ...
  it("simulated disconnect stops message routing", async () => {
    ...
    // Send a message — should throw because connection is closed
    await expect(
